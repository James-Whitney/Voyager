...
void VFC::ExtractVFPlanes(mat4 P, mat4 V) {
   /* composite matrix */
   mat4 comp = P * V;
...
float VFC::DistToPlane(float A, float B, float C, float D, vec3 point) {
   return (A*point.x) + (B*point.y) + (C*point.z) + D;
}


int VFC::ViewFrustCull(std::shared_ptr<Renderable> renderable, std::shared_ptr<Hud> hud, int i) {
